...
def find_object(sha1_prefix):
    ...
    if len(objects) >= 2:
        raise ValueError(
            f"multiple objects ({len(objects)}) with prefix {sha1_prefix!r}"
        )

    return os.path.join(obj_dir, objects[0])


def read_object(sha1_prefix) -> Tuple[str, bytes]:
...
def commit(message, author=None):
    ...
    author_time = (
        f"{timestamp} {'+' if utc_offset > 0 else '-'}"
        f"{abs(utc_offset) // 3600:02}{(abs(utc_offset) // 60) % 60:02}"
    )
    ...
    print(f"commited to master: {sha1:7}")
...
def find_commit_objects(commit_sha1):
    ...
    its tree, its parent, and the hash of the commit itself.
    """
    objects = {commit_sha1}
...
def push(git_url, username=None, password=None):
    ...
    print(
        f"updating remote master from {remote_sha1 or 'no commits'} to "
        f"{local_sha1} ({len(missing)} object{'' if len(missing) == 1 else 's'})"
    )

    lines = [
        f"{remote_sha1 or ('0' * 40)} {local_sha1} "
...
if __name__ == "__main__":
    ...
    sub_parser.add_argument(
        ...
        "(users GIT_AUTHOR_NAME and GIT_AUTHOR_EMAIL environment variables by default)",
    )
    sub_parser.add_argument(
    ...
    sub_parser.add_argument(
        "git_url",
        help="URL of git repo, eg: https://github.com/zeroam/build-your-own-x.git",
    )
    sub_parser.add_argument(
        "-p",
        "--password",
    ...
    sub_parser.add_argument(
        "-u",
        "--username",
        help="username to use for authentication (users GIT_USERNAME "
        "environment variable by default)",
    )
